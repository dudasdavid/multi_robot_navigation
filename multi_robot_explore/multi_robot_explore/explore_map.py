...
class MultiRobotExplorer(Node):
    ...
    def explore(self):
        ...
        global_frontiers_set = set(global_frontiers)
        local_frontiers_1 = [f for f in self.find_frontiers(self.local_map_1)
                             if f in global_frontiers_set and not self.is_frontier_unreachable('robot_1', f, self.local_map_1)]
        transformed_frontiers_2 = []
        resolution = self.local_map_2.info.resolution
        origin = self.local_map_2.info.origin.position
        try:
            transform = self.tf_buffer.lookup_transform(
                'world', 'robot_2/map', rclpy.time.Time(), timeout=rclpy.duration.Duration(seconds=0.5)
            )
            for f in self.find_frontiers(self.local_map_2):
                y, x = f
                local_x = origin.x + (x + 0.5) * resolution
                local_y = origin.y + (y + 0.5) * resolution
                pose = PoseStamped()
                pose.header.frame_id = 'robot_2/map'
                ...
                try:
                    ...
                    gx = int((transformed.position.x - self.global_map.info.origin.position.x) / self.global_map.info.resolution)
                    gy = int((transformed.position.y - self.global_map.info.origin.position.y) / self.global_map.info.resolution)
                    if (gy, gx) in global_frontiers_set and not self.is_frontier_unreachable('robot_2', f, self.local_map_2):
                        transformed_frontiers_2.append(f)
                except Exception as e:
                    self.get_logger().warn(f"Transform failed for robot_2 frontier {f}: {e}")
        except Exception as e:
            self.get_logger().warn(f"TF lookup failed for robot_2/map to map: {e}")
            transformed_frontiers_2 = []

        local_frontiers_2 = transformed_frontiers_2
        ...
        local_frontiers_1 = [f for f in self.find_frontiers(self.local_map_1)
                             if f in global_frontiers_set and not self.is_frontier_unreachable('robot_1', f, self.local_map_1)]
        local_frontiers_2 = [f for f in self.find_frontiers(self.local_map_2)
                             if f in global_frontiers_set and not self.is_frontier_unreachable('robot_2', f, self.local_map_2)]
